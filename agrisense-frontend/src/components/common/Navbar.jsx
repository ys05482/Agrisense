...
export default function Navbar() {
  const { t, toggle, nextLangLabel, isTranslating } = useLang();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            {/* Language cycle button — EN → हिंदी → मराठी → EN */}
            <button
              onClick={toggle}
              disabled={isTranslating}
              className="text-sm border border-primary text-primary px-3 py-1.5 rounded-full
                         font-medium hover:bg-green-50 transition shrink-0 min-w-[72px]
                         disabled:opacity-60 flex items-center gap-1 justify-center"
              title="Switch language"
            >
              {isTranslating ? (
                <span className="inline-block w-4 h-4 border-2 border-primary
